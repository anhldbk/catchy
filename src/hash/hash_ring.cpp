...
hash_ring_t *hash_ring_create(uint32_t numReplicas, HASH_FUNCTION hash_fn) {
    ...
    if (hash_fn != HASH_FUNCTION_MD5 && hash_fn != HASH_FUNCTION_SHA1 &&
            hash_fn != HASH_FUNCTION_XXHASH)
        return NULL;
...
static int hash_ring_hash(hash_ring_t *ring, uint8_t *data, uint8_t dataLen,
        uint64_t *hash) {
    if (ring->hash_fn == HASH_FUNCTION_MD5) {
        ...
        *hash = keyInt;
        return 0;
    } else if (ring->hash_fn == HASH_FUNCTION_XXHASH) {
        *hash = get_xxhash((void *) data, dataLen, 0);
        return 0;
    } else {
        return -1;
    }
...
int hash_ring_add_items(hash_ring_t *ring, hash_ring_node_t *node) {
    ...
    void *resized = realloc(ring->items, (sizeof (hash_ring_item_t *) *
...
static int item_sort(const void *a, const void *b) {
    hash_ring_item_t *itemA = *(hash_ring_item_t **) a,
            *itemB = *(hash_ring_item_t **) b;
    if (itemA == NULL)
        return 1;
    if (itemB == NULL)
        return -1;
...
int hash_ring_add_node(hash_ring_t *ring, uint8_t *name, uint32_t nameLen) {
    if (ring == NULL)
        return HASH_RING_ERR;
    if (hash_ring_get_node(ring, name, nameLen) != NULL)
        return HASH_RING_ERR;
    ...
    // Sort the items
    qsort((void **) ring->items, ring->numItems, sizeof (struct hash_ring_item_t *),
            item_sort);
...
int hash_ring_remove_node(hash_ring_t *ring, uint8_t *name, uint32_t nameLen) {
    if (ring == NULL || name == NULL || nameLen <= 0)
        return HASH_RING_ERR;
    ...
    while (cur != NULL) {
        ...
        if (node->nameLen == nameLen && memcmp(node->name, name, nameLen) == 0) {
            ...
            qsort((void **) ring->items, ring->numItems,
                    sizeof (struct hash_ring_item_t *), item_sort);
            ring->numItems -= ring->numReplicas;
...
hash_ring_item_t *hash_ring_find_next_highest_item(hash_ring_t *ring,
        uint64_t num) {
    if (ring->numItems == 0)
        return NULL;
...
hash_ring_node_t *hash_ring_find_node(hash_ring_t *ring, uint8_t *key,
        uint32_t keyLen) {
    if (ring == NULL || key == NULL || keyLen <= 0)
        return NULL;

    uint64_t keyInt;

    if (hash_ring_hash(ring, key, keyLen, &keyInt) == -1)
        return NULL;

    return hash_ring_find_node(ring, keyInt);
}

hash_ring_node_t *hash_ring_find_node(hash_ring_t *ring, uint64_t keyInt) {
    if (ring == NULL)
        return NULL;
 ...
 */
int hash_ring_find_nodes(hash_ring_t *ring, uint8_t *key, uint32_t keyLen,
        hash_ring_node_t *nodes[], uint32_t num) {
